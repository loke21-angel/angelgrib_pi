...
int angelgrib_pi::Init()
{
    ...
    wxString icon_svg = GetPluginDataDir("angelgrib_pi") + wxFileName::GetPathSeparator() + "angelgrib.svg";
...
int angelgrib_pi::GetAPIVersionMinor() { return 17; }
...
wxString angelgrib_pi::GetShortDescription() { return "Download NOAA GFS GRIB files"; }
...
bool angelgrib_pi::DownloadAndOpenGrib()
{
    ...
    wxString json;
    json.Printf("{\"grib_file\":\"%s\"}", output);
...
bool angelgrib_pi::DownloadFile(const wxString& url, const wxString& output_file)
{
    ...
    wxInputStream* input = downloader.GetInputStream();
    if (!input || !input->IsOk()) {
        delete input;
        return false;
    }
